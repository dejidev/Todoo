...
const Complete = () => {
    ...
    return (
        <div>
            <h1 className='pt-4  text-xl font-semibold '>Complete Tasks</h1>

            {/* Completed Tasks */}
            <div className="flex-grow p-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 m-auto">
                    {completedTasks.map((task) => (
                        <div key={task.id} className="bg-green-100 p-4 rounded-md shadow-md shadow-gray-300 hover:shadow-lg transition">
                            <p className="font-bold mb-2">{task.title}</p>
                            <p className="text-sm mb-2">{task.description}</p>
                            <p className="text-xs mb-2">Due Date: {task.dueDate}</p>
                            <p className="text-xs text-green-700">Status: Completed</p>
